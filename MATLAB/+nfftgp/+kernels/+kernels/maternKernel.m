function [kernel] = maternKernel(X, f, l, mu, params, require_grad, form_matrix)
%% [KMat] = maternKernel(X, f, l, mu, params, require_grad, form_matrix)
...
%           kernel formula: k(x, y) = f^2 * (1+sqrt(3)||x-y||/l) * exp(-sqrt(3)*||x-y||/l)
%
%  inputs:
...
%
%  example:
%           kernelg = nfftgp.kernels.kernels.maternKernel(X, f, l, mu, {}, 1);
%           KMatg = nfftgp.kernels.kernels.maternKernelMat(kernelg, [], []);
%           Create kernel matrix with derivative
   ...
   if(nargin < 4 || nargin > 7)
      ...
      kernel.l = 1.0;
      kernel.mu = 0.01;
      kernel.kernelfunc = @nfftgp.kernels.kernels.maternKernelMat;
      kernel.params = {};
